...
pub const enabled = blk: {
    ...
    if (!builtin.is_test) {
        const options = @import("blasttracy_options");
        if (@hasDecl(options, "enable_blasttracy")) {
            build_enable = options.enable_blasttracy;
        }
    }
...
const tracy_stub = struct {
    ...
    pub inline fn FrameMarkName(name: [*:0]const u8) void {
        _ = name;
    }
