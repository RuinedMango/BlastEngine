...
const options = @import("blastglfw_options");
...
pub fn mayberErrorString(str: *?[:0]const u8) Error!void {
...
pub const Joystick = struct {
    ...
    pub fn getGuid(self: Joystick) [:0]const u8 {
        return std.mem.span(glfwGetJoysticksGUID(@as(i32, @intCast(self.jid))));
    }
    extern fn glfwGetJoysticksGUID(jid: i32) [*:0]const u8;
    ...
    fn isGamepad(self: Joystick) ?Gamepad {
    ...
    pub const State = extern struct {
        ...
        test {
            ...
            inline for (comptime std.meta.fieldNames(State)) |field_name| {
                try std.testing.expectEqual(@offsetOf(c.GLFWgamepadstate, field_name), @offsetOf(State, field_name));
            }
        }
    };
...
pub const WindowHint = enum(i32) {
    ...
    /// Use 'windowHint', 'windowHintString' or 'windowHintTyped'
    pub const set = glfwWindowHint;
};

pub fn windowHintTyped(comptime window_hint: WindowHint, value: WindowHint.ValueType(window_hint)) void {
...
pub const ReleaseBehavior = enum(i32) {
    any = 0,
    flush = 0x00035001,
    none = 0x00035002,
};
